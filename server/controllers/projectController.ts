...
export const createProject = async (req: Request, res: Response) => {
    ...
    if (images.length < 2 || !productName) {
        return res.status(400).json({ message: 'Please upload at least 2 images and also product name' })
    }
    ...
    try {
        ...
        res.json({ message: 'Project created successfully', projectId: project.id })


    } catch (error: any) {
        if (tempProjectId!) {
            // update project to set isGenerating to false if error occurs
            await prisma.project.update({
                where: { id: tempProjectId },
                data: { isGenerating: false, error: error.code || error.message }
            })
        }
        if (isCreditDeducted) {
            // refund credits if error occurs
            await prisma.user.update({
                where: { id: userId },
...
export const createVideo = async (req: Request, res: Response) => {
    ...
    if (!user || user.credits < 10) {
        return res.status(401).json({ message: 'Insufficient credtis' })
    }
    ...
    try {
        ...
        if (!operation.response.generatedVideos) {
            // Safely check for the reason, or use a default error message
            const errorMessage = operation.response?.raiMediaFilteredReasons?.[0]
                || operation.response?.error?.message
                || 'Video generation failed for an unknown reason.';
