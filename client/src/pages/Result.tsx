...
const Result = () => {
  ...
  const handleGeneratedVideo = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || error.message);
      console.log(`handleGeneratedVideo error ${error}`);
    } finally {
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex justify-between items-center mb-8">
          ...
          <Link to="/generate" className="btn-secondary text-sm flex items-center gap-2">
            ...
            <p className="max-sm:hidden">New Generation</p></Link>
        </header>
        {/* Grid layout */}
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="glass-panel inline-block p-2 rounded-2xl">
              <div className={`${projectData?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} sm:max-h-200 rounded-xl bg-gray-900 overflow-hidden relative`}>
                {projectData?.generatedVideo ? (
                  <video src={projectData.generatedVideo} controls autoPlay loop className="w-full h-full object-cover" />
                ) : (
                  <img src={projectData.generatedImage} alt="Generated Result" className="w-full h-full object-cover" />
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="glass-panel p-6 rounded-2xl">
              ...
              <div className="flex flex-col gap-3">
                <a href={projectData.generatedImage} download >
                  <GhostButton disabled={!projectData.generatedImage}
                ...
                <a href={projectData.generatedVideo} download >
                  <GhostButton disabled={!projectData.generatedVideo}
                    className="w-full justify-center rounded-md py-3 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="glass-panel p-6 rounded-2xl relative overflow-hidden">
              ...
              {!projectData.generatedVideo ? (
                <PrimaryButton onClick={handleGeneratedVideo}
