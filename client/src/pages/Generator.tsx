...
const Generator = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'product' | 'model') => {
    try {
      ...
      console.log(`handleFileChange function error ${error}`)
    }
  }
  ...
  const handleGenerate = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      console.log(`handleGenerate function error ${error}`)
    }
  }
  ...
  return (
    <div className="min-h-screen text-white p-6 md:p-12 mt-28">
      <form onSubmit={handleGenerate} className="max-w-4xl mx-auto mb-40">
        ...
        <div className="flex gap-20 max-sm:flex-col items-start justify-center">
          {/* left col */}
          ...
          <div className="w-full">
            <div className="mb-4 text-gray-300">
              ...
              <input type="text" id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Name your project" required className="w-full bg-white/3 rounded-lg  p-4 text-sm border-2 border-violet-200/10 focus:border-violet-500/50 outline-none transition-all" />
            </div>
            <div className="mb-4 text-gray-300">
              <label htmlFor="productName" className="block text-sm mb-4">
                Product Name
              </label>
              <input type="text" id="productName" value={productName} onChange={(e) => setProductName(e.target.value)} placeholder="Enter the name of the product" required className="w-full bg-white/3 rounded-lg  p-4 text-sm border-2 border-violet-200/10 focus:border-violet-500/50 outline-none transition-all" />
            </div>
            <div className="mb-4 text-gray-300">
