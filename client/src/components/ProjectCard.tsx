...
const ProjectCard = ({ gen, setGenerations, forCommunity = false }: { gen: Project, setGenerations: React.Dispatch<React.SetStateAction<Project[]>>, forCommunity?: boolean }) => {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            console.log(`hanleDelete error ${error}`);
        }
    }
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
                    ...
                    {!forCommunity && (
                        <div
                            ...
                            <div className="flex flex-col items-end w-32 text-sm">
                                <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} verflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-2 z-10`}>
                ...
                {gen.productDescription && (
                    <div className="mt-3 ml-3">
                        <p className="text-xs text-gray-400 mb-1 ml-1">Description</p>
